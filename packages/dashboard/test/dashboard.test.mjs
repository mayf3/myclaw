...
test("dashboard serves HTML and status API", async () => {
  ...
  const [
    ...
    assert.equal(status.milestones.currentPhase, "1.9");
    assert.equal(status.experiments.currentPhase, "1.9");
    assert.equal(status.health.items.some((item) => item.id === "html-center"), true);
    assert.equal(status.health.items.some((item) => item.id === "llm-provider"), true);
