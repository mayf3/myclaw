...
test("gateway serves dashboard status and accepts inbound messages", async () => {
  ...
  try {
    ...
    assert.equal(milestones.milestones.currentPhase, "1.9");
    ...
    assert.equal(experiments.experiments.currentPhase, "1.9");
    assert.deepEqual(experiments.experiments.layerRoadmap.map((item) => item.id), ["L0", "L1", "L2", "L3", "L4", "L5", "L6"]);
    assert.equal(experiments.experiments.experiments.some((item) => item.id === "E5B"), true);
    assert.equal(experiments.experiments.experiments.some((item) => item.id === "E6A"), true);
    assert.equal(experiments.experiments.experiments.some((item) => item.id === "E6B"), true);
    assert.equal(experiments.experiments.experiments.some((item) => item.id === "E6C"), true);
