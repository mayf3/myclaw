const moduleDefinitions = [
  ...
  },
  {
    ...
    hermes: 55,
    openhuman: 90,
    gap: "已有 run detail/stage summary/stage review/approval queue/tool request/human experiments、health strip、mutation audit 和 SSE stream 状态，缺完整 review drawer 和 event seq/replay",
    next: "把 review drawer、tool request panel、自动刷新策略和事件 replay 做成一等操作面",
    criteria: [
      c("dashboard-status", "state/runs/events/channels/milestones/experiments/approvals/toolRequests/health/audit/stream view", "done", 94, "packages/dashboard/src/client.mjs"),
  ...
  },
  {
    ...
    hermes: 92,
    openhuman: 90,
    gap: "已有 approval-to-tool smoke，但还没有 agent turn、LLM tool loop、subagent、context budget",
    next: "先把 smoke tool 提升成 schema/policy dispatch，再接 LLM/tool loop",
    criteria: [
  ...
  },
  {
    ...
    hermes: 74,
    openhuman: 84,
    gap: "已有 migration approval queue 和 smoke tool approval execution，缺通用 tool schema、policy snapshot、sandbox",
    next: "把 smoke tool request 抽成插件化 ToolDescriptor + policy + sandbox dispatch",
    criteria: [
