const moduleDefinitions = [
  {
    ...
    hermes: 78,
    openhuman: 86,
    gap: "已拆 routes/auth、共享 read route adapter，并有 approval decision mutation、mutation audit、SSE snapshot 和 scoped token，仍缺 route schema、seq/replay 和 WebSocket",
    next: "增加 route schema、event seq/replay 和 approval-to-tool 边界",
    criteria: [
  ...
  },
  {
    ...
    hermes: 55,
    openhuman: 90,
    gap: "已有 run detail/stage summary/stage review/approval queue/human experiments、health strip、mutation audit 和 SSE stream 状态，缺完整 review drawer 和 event seq/replay",
    next: "把 review drawer、自动刷新策略和事件 replay 做成一等操作面",
    criteria: [
      c("dashboard-status", "state/runs/events/channels/milestones/experiments/approvals/health/audit/stream view", "done", 92, "packages/dashboard/src/client.mjs"),
