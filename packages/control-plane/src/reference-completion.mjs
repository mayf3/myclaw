const moduleDefinitions = [
  {
    ...
    hermes: 78,
    openhuman: 86,
    gap: "已拆 routes/auth、共享 read route adapter，并有 approval decision mutation、mutation audit、SSE snapshot、scoped token 和 smoke tool request，仍缺 route schema、seq/replay 和 WebSocket",
    next: "增加 route schema、event seq/replay 和通用 tool dispatch 边界",
    criteria: [
      c("gateway-http", "HTTP health/status/messages/runs/stage routes", "done", 100, "packages/gateway/src/routes"),
      c("gateway-guard", "loopback/token/scoped-token guard", "done", 90, "packages/gateway/src/auth.mjs"),
      c("gateway-adapter", "shared read control route adapter and audit read route", "partial", 88, "packages/control-plane/src/http-routes.mjs"),
      c("gateway-audit", "mutation audit without request body or token values", "partial", 75, "packages/gateway/src/audit.mjs"),
      c("gateway-stream", "SSE snapshot/heartbeat and scoped token", "partial", 60, "packages/control-plane/src/event-stream.mjs"),
    ],
  },
  {
    id: "feishu",
    ...
    hermes: 42,
    openhuman: 35,
    gap: "已有 adapter/signature/encrypted challenge/custom-bot outbound facade、openduck credential import、WebSocket app-token 群回复、default-closed local policy、persistent replay、read redaction 和 LLM reply run linking，缺 rich card",
    next: "扩展 rich card、持久 dispatch 详情和 Dashboard reply chain drawer",
    criteria: [
      ...
      c("feishu-runtime", "custom-bot outbound, app-token direct reply, WebSocket bot runtime, local policy, persistent replay and linked LLM reply run", "partial", 92, "packages/feishu-bot/src/runtime.mjs"),
    ],
  },
  {
    id: "dashboard",
    ...
    hermes: 55,
    openhuman: 90,
    gap: "已有 run detail/stage summary/stage review/approval queue/tool request/human experiments、LLM provider health、mutation audit 和 SSE stream 状态，缺完整 review drawer 和 event seq/replay",
    next: "把 review drawer、LLM run detail、tool request panel、自动刷新策略和事件 replay 做成一等操作面",
    criteria: [
      c("dashboard-status", "state/runs/events/channels/milestones/experiments/approvals/toolRequests/llm health/audit/stream view", "done", 95, "packages/dashboard/src/client.mjs"),
      c("dashboard-reference", "reference matrix, Feishu adoption and adapter readiness", "partial", 75, "packages/dashboard/src/client.mjs"),
      c("dashboard-actions", "run detail, approval queue and stage review summary", "partial", 76, "packages/dashboard/src/client.mjs"),
    ],
  },
  {
    id: "migration",
  ...
  },
  {
    ...
    hermes: 92,
    openhuman: 90,
    gap: "已有 approval-to-tool smoke、OpenAI Responses 单轮 ask、review-only agent config proposal 和 Feishu LLM reply chain，但还没有 LLM tool loop、subagent、context budget",
    next: "先把配置提案和 smoke tool 提升成 schema/policy/durable dispatch，再接模型工具调用循环",
    criteria: [
      c("runtime-envelope", "message envelope and persistence seed", "partial", 25, "packages/runtime/src/messages.mjs"),
      c("agent-ask", "single-turn LLM answer persisted as run", "partial", 45, "packages/agent/src/ask.mjs"),
      c("agent-config-proposal", "review-only config proposal with approval", "partial", 55, "packages/agent/src/config-proposal.mjs"),
      c("agent-feishu-reply", "Feishu LLM replies link fb_* runs to ask_* runs", "partial", 35, "packages/feishu-bot/src/runtime.mjs"),
      c("agent-tool-loop", "LLM tool loop with ToolDescriptor dispatch", "missing", 0, "Phase 1.9 backlog"),
  ...
  },
  {
    ...
    hermes: 74,
    openhuman: 84,
    gap: "已有 migration approval queue、smoke tool approval execution、LLM text provider smoke 和配置提案 approval，缺通用 tool schema、policy snapshot、sandbox",
    next: "把配置提案和 smoke tool request 抽成插件化 ToolDescriptor + policy + sandbox dispatch，再给 LLM 暴露裁剪后的工具",
    criteria: [
      c("mutation-guard", "gateway mutation and scoped-token guard", "partial", 58, "packages/gateway/src/auth.mjs"),
      c("approval-state", "approval as first-class state", "partial", 62, "packages/core/src/approvals.mjs"),
      c("tool-approval-smoke", "safe local tool request executes only after approval", "partial", 55, "packages/tools/src/smoke-note.mjs"),
