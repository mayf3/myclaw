const moduleDefinitions = [
  ...
  },
  {
    ...
    hermes: 92,
    openhuman: 90,
    gap: "已有 approval-to-tool smoke 和 OpenAI Responses 单轮 ask，但还没有 LLM tool loop、subagent、context budget",
    next: "先把 smoke tool 提升成 schema/policy dispatch，再接模型工具调用循环",
    criteria: [
  ...
  },
  {
    ...
    hermes: 74,
    openhuman: 84,
    gap: "已有 migration approval queue、smoke tool approval execution 和 LLM text provider smoke，缺通用 tool schema、policy snapshot、sandbox",
    next: "把 smoke tool request 抽成插件化 ToolDescriptor + policy + sandbox dispatch，再给 LLM 暴露裁剪后的工具",
    criteria: [
