const moduleDefinitions = [
  {
    ...
    hermes: 78,
    openhuman: 86,
    gap: "已拆 routes/auth、共享 read route adapter，并有 approval decision mutation 和 mutation audit，仍缺 WS/event stream、scoped token、route schema",
    next: "增加 scoped token 和 event stream",
    criteria: [
  ...
  },
  {
    ...
    hermes: 55,
    openhuman: 90,
    gap: "已有 run detail/stage summary/stage review/approval queue/human experiments、health strip 和 mutation audit，缺实时事件和完整 review drawer",
    next: "把 event stream 和 review drawer 做成一等操作面",
    criteria: [
      c("dashboard-status", "state/runs/events/channels/milestones/experiments/approvals/health/audit view", "done", 90, "packages/dashboard/src/client.mjs"),
