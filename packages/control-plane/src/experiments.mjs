export function buildHumanExperimentsPayload() {
  const experiments = [
    ...
    experiment({
      ...
      successSignals: ["Human Experiments 区块可见", "当前 Phase 显示 1.8", "每个实验都有命令和成功信号"],
  ...
  return {
    ...
    layerRoadmap: [
      ...
      layer("L3", "单 Agent Runtime", "一次任务能拆解、执行、失败重试、人工确认和验收；当前有真实 LLM 单轮回复和 review-only 配置提案。", "partial", ["E6A", "E6B", "E6"]),
    ...
    planningMap: [
      ...
      step("M4", "审批与安全", "E5/E5B/E6A/E6B", "立即可测"),
      step("M5", "Agent Runtime 与记忆", "E6", "LLM 回复和配置提案已可测，工具循环后续开放"),
