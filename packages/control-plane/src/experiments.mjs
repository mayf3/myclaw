export function buildHumanExperimentsPayload() {
  const experiments = [
    ...
    experiment({
      ...
      successSignals: ["Human Experiments 区块可见", "当前 Phase 显示 1.9", "每个实验都有命令和成功信号"],
    ...
    experiment({
      ...
      nextUnlock: "E6B agent config proposal",
    }),
    experiment({
      id: "E6B",
      milestone: "M4",
      title: "Agent Config Proposal Smoke",
      status: "ready",
      role: "配置审阅者",
      whatToTest: "用 agent 生成 Feishu LLM 配置提案，并确认它只创建 review-only approval，不写配置、不输出 secret。",
      commands: [
        'export OPENAI_API_KEY="sk-..."',
        'npm run myclaw -- configure-agent --target feishu-llm --text "帮我检查飞书 LLM 回复还差哪些配置" --json',
        "curl -s http://127.0.0.1:4321/api/approvals",
        "打开 Dashboard 查看 cfg_* run 和 pending approval",
      ],
      successSignals: [
        "`cfg_*` run 写入 state",
        "result.type=agent-config-proposal 且 capabilities.appliesChanges=false",
        "pending approval 的 subject.type=agent-config-proposal",
        "Dashboard/API 只暴露 proposalPreview，不暴露 secret、chat_id 或完整 proposal",
      ],
      nextUnlock: "E6C Feishu LLM reply chain",
    }),
    experiment({
      id: "E6C",
      milestone: "M4",
      title: "Feishu LLM Reply Chain Smoke",
      status: "ready",
      role: "飞书真实回复测试者",
      whatToTest: "在备份飞书群发送文本，确认 MyClaw 用 LLM 直接群回复，并且 Dashboard/API 能看到 fb_* run 关联到 ask_* run。",
      commands: [
        'export OPENAI_API_KEY="sk-..."',
        "mkdir -p .myclaw && printf '%s\\n' '{\"allowedChatIds\":[\"oc_xxx\"]}' > .myclaw/feishu-policy.json",
        "set -a; source .myclaw/openduck-feishu.env; set +a; MYCLAW_FEISHU_LLM_ENABLED=1 npm run myclaw -- feishu-bot --reply-provider llm --llm-privacy-ack --reply-mode direct",
        "在备份飞书群发送一条文本消息",
        "打开 Dashboard 查看最新 fb_* run，确认 reply.builder.linkedRunId 指向 ask_*",
      ],
      successSignals: [
        "飞书群收到直接回复，不是话题回复",
        "`fb_*` run 写入 state，且 `reply.builder.provider=llm`",
        "`fb_*` run 的 `reply.builder.linkedRunId` 指向一个 `ask_*` run",
        "Dashboard/API 仍隐藏飞书原文、chat_id、sender_id 和完整 LLM answer",
        "缺少 OPENAI_API_KEY 时记录 feishu.bot.reply.failed，不伪造回复",
      ],
  ...
  return {
    ...
    layerRoadmap: [
      ...
      layer("L3", "单 Agent Runtime", "一次任务能拆解、执行、失败重试、人工确认和验收；当前有真实 LLM 单轮回复、review-only 配置提案和 Feishu LLM 回复链路。", "partial", ["E6A", "E6B", "E6C", "E6"]),
    ...
    planningMap: [
      ...
      step("M4", "审批与安全", "E5/E5B/E6A/E6B/E6C", "立即可测"),
      step("M5", "Agent Runtime 与记忆", "E6", "LLM 回复、配置提案和 Feishu LLM 回复链路已可测，工具循环后续开放"),
